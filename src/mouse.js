...
export function createMouse(d) {
  return {
    ...
    onLeftClickTile: (tile, point, sx, sy) => {
      ...
      // In d.combat a click resolves in the same order the d.hover affordances do:
      ...
      // click does.
      if (d.inCombat) {
        // Initiative: you control whoever's turn it is - a d.party member or a
        // summon you conjured. d.combat.actingActor is that body (d.party.active
        // can't point at a summon, which lives outside the roster). Clicks
        // target d.enemies or drive that unit; no switching (each acts on its
        // own turn).
        ...
        // ungated, it would eat the clicks that walk you past your own d.party.
        ...
        // switches the leader out of d.combat. Steering only ever succeeds on a
        ...
        // Ground fallback: the same near-a-body test the d.hover affordances
        // run (d.combat.enemyAtPoint), so a click can't route into a walk on a
        ...
        // the door instead of working it. d.toggleDoor owns the rules from here:
      ...
      // placement d.grid and nothing else: the click posts the role where you
      ...
      // says why and stays armed (d.postSummonAt), so the next click can just be
      // a better one.
      ...
      // and the ground branch only ever handled d.summons, so aiming Bulk Mail at
      ...
      if (d.armedOoc && d.ACTIONS[d.armedOoc].cone && point) {
        ...
        // From the BODY, like the preview and the in-d.combat wedge - one
      ...
      // Out of d.combat, the interactable ENTITY under the cursor wins over the
    ...
    onHover: (point, sx, sy) => {
      if (d.inCombat && d.combat) {
        ...
        // The d.hover had no such rule, so on those pixels the crosshair and
        ...
        // swings the basic attack (d.combat.js), so the cursor has to say so.
        // d.combat.handleHover resolves WHO that is (this body pick first, the
        ...
        // (d.combatDoorAt) and handed to d.combat alongside the d.hover - the
        ...
        // so with the same pointer it uses out of d.combat. The click reads the
        ...
        // the DOS2 read, and the same one you already get out of d.combat. This
        ...
        // A foe the d.hover resolved through the GROUND fallback (the pick ray
      ...
      // Layered: the d.hover point follows the same top-down storey pick as the
      ...
      // d.hover events only arrive when the mouse actually moves.
      d.setOocAim(point);
      d.hover.hover(point, sx, sy);
    },
    // The cursor left the world for the DOM UI: drop the world d.hover rather
    // than leaving the last-hovered body glowing and named behind the panel
    // the d.player is now using.
    onHoverLeave: () => {
      d.hover.clear();
    ...
    onRightClickTile: (tile, sx, sy, point) => {
      if (!d.sheet || d.gameOver) return;
      // In d.combat, right-click is first the universal "back out": it lowers an
      ...
      if (d.inCombat) {
        ...
        // in-d.combat sibling of the out-of-d.combat "Switch to" below.
