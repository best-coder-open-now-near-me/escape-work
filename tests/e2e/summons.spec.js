...
test('the Manager escalates AT a spot you pick', async ({ page }) => {
  ...
  // Escalate is on the bar, and no employees have shown up yet.
...
test('Escalate is on the out-of-combat bar, and posts where you click', async ({ page }) => {
  test.setTimeout(300_000);
  await bootStash(page, QUIET_ARENA, 'middle-manager');

  // The whole kit is listed out of combat, not just the slot we are about to
  // press. The Manager's kit is exactly these two (classes.js: delegate,
  // escalate) since POWERS_PLAN M9 moved the summon to him and took his heal
  // away. This used to boot HR and ask for #hotbar-act-defend, which no HR has
  // ever had - the assertion could only ever have failed, and it sat in the
  // tail of the suite that CI's maxFailures cap kept it from ever reaching.
  await expect(page.locator('#hotbar-act-escalate')).toBeVisible();
  await expect(page.locator('#hotbar-act-delegate')).toBeVisible();
  expect(await page.evaluate(() => window.__game.summons.length)).toBe(0);
...
test('an out-of-range spot is refused, and the posting is not spent', async ({ page }) => {
  test.setTimeout(300_000);
  await bootStash(page, LONG_ARENA, 'middle-manager');
  await enterCombat(page);
