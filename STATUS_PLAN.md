...
| 2 | ~~Combat-scoped turn statuses~~ **Superseded — one clock, in and out** `[stated]` (designer, 2026-07-31) | Turn-clock statuses **keep ticking out of combat**, on main.js's world clock — the same clock that already ages fire, smoke, summon assignments and dropped litter. Nothing is swept at combat end; step-clock statuses persist as they always did | The premise ("there are no turns outside combat") stopped being true the day the world clock landed: it already spends everything a combat round spends, and statuses were the one thing it did not. The designer's terms: *"that clock should've been used from the beginning for tracking fire and player effects. those should all be using the same thing in and out of combat, its not something new going on here."* The old sweep was doing real harm on its way out — walking out of a fight put out the fire you were carrying and handed back a Deflect you had spent. `covered` is the one exception, and it always was: its duration is a leak bound, not a clock, so main.js re-applies it while the out-of-combat crouch holds, exactly as combat re-applies it while the in-combat one does. |
...
| 9 | ~~Fear/charm deferred~~ **Superseded — charm shipped, fear did not** | Charm shipped as Phase 8's `remote-session` (IT track node `it-remote`): 3 AP, `applies: 'charmed'`, 3 turns, resistable, one use. Fear (forced flee) is still **designed but not shipped**. | The shipped charm is not this row's `mandatory-fun` team-swap and needed none of the AI hooks the row named. `combat.js` BORROWS the unit instead: `charmUnit` synthesizes a sheet, pushes the body into `members`, and `turns.replace` swaps the initiative slot in place, so the player drives it on an ordinary player turn — BG3 Dominate rather than DOS2's AI-driven Charmed (framing carried from the queue's own note; not re-verified against either game here, so treat the comparison as memory, not a checked fact). `pickTarget`/`aiAdvance` were never touched; the one thing that had to be built was `turns.replace`, the inverse `insert` never had. The `charmed` status entry carries no `effects` on purpose: it changes which SIDE somebody is on, not a stat. Fear still needs forced-move resolution in the AI driver, so it stays deferred. |
...
- Combat end (`cleanup`): ~~sweep turn-clock statuses from every combatant~~ —
  **no sweep** (decision 2, superseded). Statuses walk out of the fight still
  counting down, on the world clock.
