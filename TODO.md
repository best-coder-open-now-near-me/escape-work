...
- [ ] **Q037** `src/combat.js:76` [god-method] **(carried)** The two god closures measured: `startCombat` is 5,065 lines / 176 inner functions / 21 shared mutable variables, `startGame` 4,214 / 159 / 26 — and combat.js grew 493 lines on this branch<br>      ↳ **CUT, not closed — 2026-08-03, and the numbers are the point.** `startCombat` 5,506 → 4,965 lines, 108 → 92 inner functions, **21 → 13 shared mutable variables**; `startGame` 4,407 → 4,133, 87 → 83, **26 → 22**. Files: `combat.js` 5,590 → 5,050, `main.js` 4,802 → 4,531. Four modules came out, all node-importable: `combat-aim.js`, `combat-world.js`, `hotbar-host.js`, and the AI turn's two halves in `combat-ai.js`.
      ...
      **Later the same day, two more slices:** `floor-effects.js` (what the floor does to a body - the per-step surface effects, the slip roll, the out-of-combat turn clock) and `combat-strikes.js` (one body hitting or moving another - `unitStrikesMember`, `opportunityStrike`, `displaceBody`, `dropOnto`, `landStun`). The second is the largest cut yet that writes NONE of the closure's shared turn state, which is exactly why it could move: 282 lines, 37 deps, and the moved text is provably identical to the original modulo the `d.` prefix (checked, not assumed). Running totals: `startCombat` 5,506 → 4,723 lines and 108 → 87 functions; `startGame` 4,407 → 4,074 and 87 → 80; `combat.js` 5,590 → 4,809, `main.js` 4,802 → 4,473.

      **Four bug classes this pass produced, all of which BUILT CLEAN and only failed in a browser.** Written down because they are what a mechanical move costs, and each now has a cheap static check:
      1. **Setters as object-literal methods.** Moved code called `setPreview(...)`; the name only existed as a property of the returned object. Killed the frame loop, which stalls every AI turn.
      2. **A key shadowing an outer function.** Inside the world facade, `findPath: (...) => findPath(...)` no longer resolves to main.js's function. Same for a spread - `[...party.members]` hid from the rewrite because the `...` reads as property access to a `(?<![.\w])` lookbehind.
      3. **A local deleted with its block.** `const show = ...` lived in the hotbar's per-frame block; the aim rings BELOW it still read it. Scan the code left behind, not just the code moved.
      4. **A rule passed as a VALUE, not called.** `truncateByBudget(s, budget, stepCost)` - the `name(` → `ask.name(` rewrite walks straight past it.
      And one that a check caught BEFORE any run: **TDZ**. Half of a closure's helpers are `const` arrows declared below the wiring, so passing them by reference reads them in their dead zone. Wrap every entry in the bag.
