...
| 6 | When you can swap | **Outside combat and during the acting character's combat turn** `[stated]` (designer task, 2026-08-06: "add the ability to equip things in combat"). A combat equip or stow costs **2 AP** `[proposed]`, the existing planned `EQUIP_AP` default. | The shipped combat-AP seam already bills pocket consumables and refreshes the shared bar. Equipment now uses that same seam: validate first, charge the acting member, mutate, then repaint derived stats and the weapon action. Two AP makes the swap a real turn choice without charging a full action. |
...
5. **Stretch — on-hit procs + the shoes slot.** ✅ Landed. **Procs** (decision
   ...
   - **In-combat equipment swap** ✅ Landed. The acting character may equip or
     stow any gear slot for 2 AP; refused verbs are atomic, and a successful
     weapon swap rebuilds the live action bar and seeds any new per-fight use
     counter. Combat availability is `[stated]` (designer task, 2026-08-06);
     the 2 AP price remains `[proposed]` until explicitly ratified.
     The cross-cutting numbers/balance pass all three plans defer to playtest
     remains open.
