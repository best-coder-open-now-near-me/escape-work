...
export function startCombat({ app, party, engaged, world, fx, callbacks, opening = null, allies = [], preCrouch = null, sneakOpened = null, rng = Math.random }) {
  ...
  const turns = createTurnOrder({
    ...
    host: {
      ...
      roundStart: () => {
        ...
        bout.rounds += 1;
        clearReactions(); // everyone gets their reaction back (TACTICS_PLAN M2)
        callbacks.onRound?.();
      },
      turnStart: () => {
  ...
  // --- opportunity attacks and overwatch (combat-reactions.js) ---------------
  // The rule and its two maps live in their own file; what stays here is the
  // wiring. `reactions` and `moveStart` went with it because nothing outside
  // that file writes either one - `clearReactions` is how the engine's
  // roundStart still refills everybody (see the turn-order callbacks above).
  const {
    canReact, combatantFor, beginMove, threatsAgainst,
    notifyStep, opportunityStrike, retireStaleMoveStarts, clearReactions,
  } = createReactions({
    get phase() { return phase; },
    TACTICS,
    MISS_COLOR,
  ...
  window.__combat = createCombatHandle({
    get phase() { return phase; },
    ...
    setForceHit: (v) => { forceHit = v; },
    setForceProc: (v) => { forceProc = v; },
    bout,
    engaged,
    members,
    watching,
