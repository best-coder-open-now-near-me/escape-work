...
| **I5** | What `depth` does to the enemies actually placed. | `scaleEnemy` (`stats.js:338-361`) recomputes hp, ap, per-attack min/max, accuracy (capped) and xp; `effectiveLevel` (`:366`) floors at the native tier. The editor draws each enemy as a flat coloured box. |
