...
- [x] **Q904** `src/stats.js` [design] **(new 2026-08-03, from the Q136 answer)**
  **DONE — [ratified]** (designer, 2026-08-05: "let's do 5 + (5 * grit)",
  correcting the earlier `5 + Grit` calibration).
  ...
  The calibration is 5 + (5 * effective Grit). The five-Grit Office Drone has
  30 slots, and the current playable classes span 20-45 slots.
...
- [ ] **Q037** `src/combat.js:97` [god-method] **(carried)** `startCombat` and `startGame` remain the central mutable combat/run closures: about 2,962 and 2,653 lines from their entry points, respectively.<br>      ↳ **PARTLY DONE.** The historical measurements below record the successful extractions; they are not current size metrics. The remaining work is to assign ownership of the shared combat and run state before attempting another extraction.
      ...
      **ADVANCED 2026-08-05:** `combat-intent.js` now owns `armed` and
      ...
      shared state is `phase`, `acting` and `active`; the run state above is
      unchanged.
      **ADVANCED AGAIN 2026-08-05:** the 153-line `window.__combat` projection
...
- [x] **Q076** `src/data/items.js:373` [dead-code] Two of five loot tables are unreachable on the shipped campaign, and `night-thermos` is unobtainable in the whole game<br>      ↳ **DONE — made them campaign content, not deleted data.** Level 1 and level 2 now each place a break-room fridge and a filing cabinet by replacing an existing solid prop, so walkability and the E2E-authored map shape stay unchanged. A campaign-chain lint now rejects every declared loot table that no shipped floor can reach, and the item-obtainability lint counts only those reachable tables. That closes the original `night-thermos` hole and prevents a dead table from falsely making an item look obtainable again.
...
- [x] **Q078** `tests/e2e/helpers.js:92` [doc-drift] The e2e seed-helper comment overclaims that `?seed=` controls slips and every in-fight roll; the corrected main.js comment is no longer the issue.<br>      ↳ **DONE — the helper now states the actual seam.** A seed pins the first fight's combat-owned hit, damage, AI line, enemy-slip, and initiative draws. It explicitly excludes member slips and loot, and warns that later fights continue the same stream rather than resetting it.
...
- [x] **Q091** `src/main.js:2616` [duplication] The out-of-combat crouch is a full parallel implementation of combat's, refusal strings included<br>      ↳ **DONE — retained the shipped pre-combat crouch, removed its second ruleset.** `crouch-rules.js` now owns the refusal ladder, prop/body cover-cell test, shielded-face composition, and the enter/leave state transition (record, pose, and status together). Combat and exploration only adapt their body lookup and retain what genuinely differs: AP and turn effects in combat, walking and HUD narration on the map. Five unit tests pin the common contract and the seven-test cover browser suite drives both sides, including a crouch taken before initiative and handed into the fight.
...
- [x] **Q093** `src/god.js:118` [god-method] `god.js buildPanel` remains an approximately 617-line closure with the debug panel's shared state and e2e-facing verbs.<br>      ↳ **DONE — explicit panel owners.** `createGodPanelState` now owns open/tab/internals/placement/pin/live-row structure, `createGodTargets` owns the reflected player/enemy/combat view models and their invariant-preserving actions, and `createGodTabs` owns the bespoke World/Spawn/Status controls plus their persistent picker state. `buildPanel` is down from roughly 617 to 366 lines and remains the DOM compositor rather than the owner of every debug concern. Five focused unit tests pin target identity, derived-field protection, placement/signature behavior, and row focus; the five-test browser suite now also drives World readouts and status apply/persist/clear through the real panel.
...
- [x] **Q107** `src/party.js:50` [inconsistency] A member downed mid-fight earns no XP for the rest of that fight, contradicting the design's own "nobody lags"<br>      ↳ **DONE — [ratified]** (designer, 2026-08-05: "q107 a" — full share; going down costs you the fight, not the campaign). `gainXpAll` drops the living-only filter: every member earns the full amount, downed included. The filter was harmless when three automatic revives stood you back up mid-fight; with all three struck, it had compounded into "miss the whole fight's XP and fall behind, which gets you downed more easily" — the spiral `gainXpAll` exists to prevent. One guard added so the answer doesn't smuggle a revive back in: `gainXp`'s promotion full-heal now skips anyone at 0 HP — a promotion earned while down banks its points, and the only way up remains a hand up (`helpUp`).
...
- [x] **Q111** `src/hover.js:211` [soc] With an armed out-of-combat action, valid non-enemy targets still fall back to the default cursor even though clicking can consume the action; enemy and ground affordances are already covered.<br>      ↳ DONE — the hover owner now asks the out-of-combat targeting rule about party hits too; Reboot gets the valid-target crosshair and the party e2e pins the handoff.
- [x] **Q112** `src/ui/hud.js:319` [soc] The HUD still owns a separate food/ammo affordance path (`fed` plus `setInert`), while `hotbar-model` owns action affordability and refusal reasons; make the presentation follow one owner.<br>      ↳ **DONE — the bar renders an availability view-model.** Out-of-combat actions now expose one state containing affordability, resource availability/count, and the exact refusal; both the renderer and the press path consume it. Combat supplies the same fields from `actionState`. The HUD only maps those facts to opacity, count text, and ARIA state, and a paper change rebuilds the view-model instead of repainting against stale rule state.
...
- [x] **Q118** `tests/e2e/summons.spec.js:45` [test-gap] The HR summon-cap assertion passes vacuously: it sleeps 1500 ms and asserts <= 2 without ever establishing that HR got another turn<br>      ↳ DONE — the test drives player turns and polls the combat round counter; a missing combat no longer satisfies the wait.
...
- [x] **Q120** `tests/unit/combat-ai.test.js:139` [test-gap] AI.W_PATH - "the baseline everything trades against" - is pinned by no assertion; the three "the cheap route wins" checks pass on fixture array order<br>      ↳ DONE — a reverse-ordered fixture proves the lower path cost wins.
- [x] **Q121** `tests/unit/combat-ai.test.js:429` [test-gap] The lineWeights assertion compares the result against AI.STATUS_WEIGHT itself, so setting the knob to 1 disables the M6 status weighting with the suite still green<br>      ↳ DONE — the expected weights are literal outcomes, independent of the implementation constant.
- [x] **Q122** `tests/unit/combat-ai.test.js:123` [test-gap] scoreDestination's backstab term and slip term are never exercised - no test passes `facing` or `slipChanceAt`, so both weights can be zeroed with the suite green<br>      ↳ DONE — independent destination fixtures now make each term decide a tie.
- [x] **Q123** `tests/unit/combat-ai.test.js:79` [test-gap] pickTarget's kill-securability and fragility terms mutually mask - neither is individually pinned, so the M2 kill term can be deleted with every test green<br>      ↳ DONE — separate target fixtures pin kill-securability and fragility without mutual masking.
- [x] **Q124** `tests/unit/combat-ai.test.js:239` [test-gap] Advance-route coverage now exercises adjacent shuffles and refusals, but the no-route/non-adjacent fixture still uses a target-insensitive `approach` stub; make the fake prove the intended guard rather than only its output.<br>      ↳ DONE — the fixture counts calls and proves the adjacent-only fallback is never consulted at distance.
- [x] **Q125** `tests/unit/combat-plans.test.js:211` [test-gap] The suite now covers several plan branches; remaining mutation gaps include `aiEdgeTopplePlan` with `terrainOpen` false and the unpinned AI-support guards.<br>      ↳ DONE — the closed-edge topple guard and support's absent-spec, invalid-HP, threshold, and blocked-line guards are all pinned.
- [x] **Q126** `tests/unit/levels.test.js:201` [test-gap] No lint requires `leavesTurns` alongside `leaves`, so the permanent-terrain bug can recur silently<br>      ↳ DONE — every terrain-leaving action must declare a finite positive duration.
...
- [x] **Q155** `src/combat.js:4829` [soc] **(carried)** The `window.__combat` / `__game` / `__god` debug surfaces are ~420 lines living inside the two god closures, and `__game.classes` hands out the live registry<br>      ↳ **DONE — `combat-debug.js`, `game-debug.js`, `god-debug.js`.** The three public adapters now own their projections and named mutation doors; the combat and run closures only supply live dependencies. `__game.classes` returns a fresh deep data snapshot, so a console or test mutation cannot rebalance the live registry. Nine focused unit tests pin snapshots, AP/status/charm doors, god-mode cash/inventory/steering/revival and placement hooks; all 915 unit tests plus the `god`, `creation` and combat-bar browser suites pass.
...
- [x] **Q167** `src/statuses.js:163` [bug] A status id that leaves the registry is immortal on an existing save: never ticked, never expired, invisible to both sweep options<br>      ↳ **DONE, with a red-first test.** Unknown legacy ids are dropped on sight by either clock tick and by the full purge. The unused filtered sweeps named in the historical mechanism were subsequently removed under Q176, so no second classification path remains.
...
- [x] **Q169** `src/combat-geometry.js:176` [dead-code] edgeShieldedTile is dead and encodes a cover rule the game no longer uses<br>      ↳ **DONE — deleted with its isolated unit test.** Live cover derives shielded faces through tactics/data; keeping an uncalled yes/no edge predicate preserved an obsolete alternate rule.
- [x] **Q170** `src/combat.js:3568` [dead-code] The purge self-cast branch in `handleTileClick` is unreachable dead code<br>      ↳ **DONE — deleted.** Reboot declares `side: 'any'`, so `aimsAtAlly` routes self-casts through the earlier friendly/any-side branch and `performBuff`, which owns billing, uses, purge, narration, and teardown. The later hand-rolled branch could never run.
- [x] **Q171** `src/combat.js:3811` [dead-code] `commitInstant`'s heal arm and `INSTANT_CONFIRM`'s 'heal' entry are dead - no `type: 'heal'` action exists<br>      ↳ **DONE — removed both dead dispatch arms.** Instant confirmation now names only the shipped self-actions (`defend`, `stance`); healing is the live targeted `buff` verb or a pocket item.
- [x] **Q172** `src/data/actions.js:13` [dead-code] `type: 'heal'` is dead in three code sites and still documented as a live verb in the action registry header<br>      ↳ **DONE with Q171/Q188.** The action bar, out-of-combat refusal table, registry docs, and speculative tests no longer preserve a verb type no action can declare.
...
- [x] **Q176** `src/statuses.js:184` [dead-code] `clearStatuses`'s `clock` and `harmfulOnly` options have no production caller, and the doc above them describes a combat-end sweep that no longer exists<br>      ↳ **DONE — `clearStatuses` is one full-purge operation.** Clocked expiry remains with `tickTurn`/`tickStep`; reboot and debug clear everything. The unused filters, their fictional combat-end documentation, and tests for the unshipped API are gone.
...
- [x] **Q180** `/home/user/escape-work/AI_PLAN.md:629` [doc-drift] AI_PLAN's architecture section places `entrenchPlan`, `shootPlan` and `supportPlan` in `combat-plans.js`; none of the three exists there<br>      ↳ **DONE — documented the landed ownership, including the deliberate absences.** `combat-ai.js` owns `aiSupportPlan` and derives shoot/entrench from shared host facts inside `aiBeatPlansFrom`; `combat-plans.js` owns only physical displacement/demolition plans; `combat-advance.js` owns the world-heavy walk; `combat.js` adapts and performs. The milestone brief now names those same seams instead of instructing an executor to create functions the implementation never needed.
- [x] **Q181** `/home/user/escape-work/AI_PLAN.md:136` [doc-drift] AI_PLAN's file:line citations are stale throughout, including the "Do not touch" list and the Brief that milestone 7's executor is told to follow<br>      ↳ **DONE — symbol references replace every volatile source line citation in AI_PLAN.** The brief points milestone 7 at `AGENTS.md`, current module owners, `aiBeatPlans`, `startCombat.update`, and the As-landed record; it explicitly labels the pre-plan tables and milestone scopes as history rather than implementation instructions.
- [x] **Q182** `/home/user/escape-work/AI_PLAN.md:202` [doc-drift] Three `[ratified]` tags in AI_PLAN cite nothing, and the twelve-arm ladder order — the doc's declared centerpiece — carries no tag at all<br>      ↳ **DONE — provenance is attached where the claims are repeated.** The crouch/attack claims cite the designer's 2026-07-30 "go with your defaults" verdict and `TACTICS_PLAN.md` M6; break-above-crouch cites the 2026-08-03 answer. The complete ladder is tagged `[ratified]` by its merged M4/M6 implementation commits and its later tested extraction, as allowed by the repository's ratification rule.
- [x] **Q183** `/home/user/escape-work/AI_PLAN.md:483` [doc-drift] AI_PLAN's published `AI` tunables block and scored-choice signatures do not match the shipped ones<br>      ↳ **DONE — the published block now matches `combat-ai.AI`, including `RANGE_SLACK` and the slip-probability meaning.** `pickTarget` and `scoreDestination(routes, q = {})` now show their real signatures and option vocabulary, the focus-0 wounded tie is described accurately, and the shipped enemy focus/ranged/support data includes its actual cooldown and log.
- [x] **Q184** `/home/user/escape-work/POWERS_PLAN.md:735` [doc-drift] POWERS_PLAN's "Open after M9: the enemy HR Representative" still says the support-enemy option is blocked by risk 7; AI_PLAN M6 shipped it<br>      ↳ **DONE — the section is closed with the ratified Q4-A result.** Enemy HR retains her summon and gained rationed healing, friendly target selection, range/LOS gates, cooldown and backline bias. Risk 7 is narrowed to what is truly still deferred: generic action-driven buffs/mobility and repositioning specifically to support. The `from: 'escalate'` oddity is recorded as naming debt, not a missing capability.
...
- [x] **Q188** `src/data/actions.js:13` [doc-drift] data/actions.js documents a `heal` action type nothing uses and omits four types that exist; three dead `heal` branches remain in code<br>      ↳ **DONE — registry and dispatch now agree.** The dead `heal` type and branches were removed; the type header now documents the four shipped omissions: `shove`, `cover`, `purge`, and `zone`.
...
- [x] **Q190** `src/main.js:1514` [doc-drift] `oocFriendlyOn`'s header promises it spends the action's `uses`; the body spends nothing<br>      ↳ **REFUTED against the extracted owner.** `oocFriendlyOn` now lives in `ooc-verbs.js`; neither its function nor the module header promises a use. Reboot declares no `uses`, so there is no ration to spend. The old cited comment did not survive the ownership extraction and no current contradiction remains.
- [x] **Q191** `src/shop.js:17` [doc-drift] shop.js names `matches` and `half-sandwich` as the items worth nothing; both carry a `value` and one is stocked by a merchant<br>      ↳ **DONE.** The zero-value example is now the real one: cash pickups are banked immediately and never reach a sellable inventory, so they need no item value. Matches and the sandwich retain their live prices.
...
- [x] **Q193** `src/ui/hud.js:32` [doc-drift] The HUD status chip drops the remaining count its own comment and data/statuses.js both promise<br>      ↳ DONE — status chips render their remaining turns/steps, with the gum lifecycle pinned in-browser.
- [x] **Q194** `src/ui/hud.js:513` [doc-drift] One comment left stranded by the `ui/` split still describes the allocation screen after the HUD module's final function.<br>      ↳ **DONE.** The stale allocation contract was removed from the HUD tail; the boundary now points modal allocation to `screens.js`.
- [x] **Q195** `src/ui/panels.js:344` [doc-drift] The shop view's documented shape omits `id`, the field the sell button's own guard depends on<br>      ↳ **DONE.** The documented sellable row now includes `id` alongside its pocket index and display/payment fields.
...
- [x] **Q197** `src/combat-geometry.js:25` [duplication] cheb is defined and exported twice, from tactics.js and combat-geometry.js, with its own test in each suite<br>      ↳ **DONE — `tactics.cheb` is the sole owner.** Combat geometry no longer defines or exports a second metric, its duplicate test is gone, and AI/plans/main import the one shared distance directly.
- [x] **Q198** `src/combat-geometry.js:37` [duplication] The four/eight-neighbour offset arrays are declared in seven places under five names<br>      ↳ **DONE — one dependency-free direction registry.** `directions.js` owns cardinal, diagonal, and composed eight-neighbour order as frozen arrays. Combat, pathfinding, grid conduction, surfaces, lint, preview, editor, loot grouping, and renderer propagation all import it; no source module re-declares those literals.
...
- [x] **Q201** `src/combat.js:2421` [duplication] "Name the thing covering you" exists three times, twice inside combat.js, with different formatting<br>      ↳ **DONE — `cover-names.js` owns the vocabulary and list formatting.** Combat crouch narration, shot/pull labels, and the out-of-combat crouch now provide only their world-specific body/tile lookups. Bodies, props, partitions, articles, deduplication, and conjunctions have one implementation with unit coverage.
- [x] **Q202** `src/combat.js:2171` [duplication] "Is a living member standing here?" is written six times inside combat.js in three shapes<br>      ↳ **DONE — `member-rules.js` owns member liveness and tile occupancy.** Combat targets/slots, route blocking and smoothing, cover denial, demolition victims, ground-zone occupancy, map party/summon occupancy, debug views, and leader handoff all derive from `isLivingMember` or `livingMemberAt`; `standing` remains deliberately narrower because toppled bodies are alive but not upright.
...
- [x] **Q204** `src/main.js:152` [duplication] mulberry32's mixer is now implemented twice — a new copy in main.js beside the existing one in combat.js<br>      ↳ **DONE — `rng.js` owns Mulberry32.** Its uint stream drives the deterministic action-bar shuffle; the Math.random-shaped float stream drives seeded fights. Two unit tests pin replay, divergence, range, and the shared mixer sequence.
- [x] **Q205** `src/main.js:667` [duplication] "Whose sheet am I steering?" is written four ways in main.js, while the actor half has a single named owner<br>      ↳ **DONE — `steeredSheet` is the sheet twin of `steeredActor`.** Pockets, HUD painting/synchronization, and the hotbar host all ask that accessor; initiative supplies the acting sheet in combat and the leader remains the out-of-combat answer.
- [x] **Q206** `src/main.js:2472` [duplication] The "who joins this fight" filter is written three times in main.js, one of them commented as a copy<br>      ↳ **DONE — the reported copies no longer exist.** `combat-entry.js` owns proximity discovery and the engaged-set handoff, and both call the shared `canTakePart` line rule plus `combat-geometry.engagedAround`. Sneak detection supplies its separately meaningful sight set; no parallel join filter remains in main.js.
- [x] **Q207** `src/main.js:2495` [duplication] The cone's "don't carpet a teammate" guard is spelled two ways, and the out-of-combat one misses summons<br>      ↳ DONE — both combat paths use the same player-side occupancy rule and the out-of-combat population includes summons.
- [x] **Q208** `src/main.js:1940` [duplication] liveSummonsOf exists in two modules counting two different populations<br>      ↳ **DONE — one cap counter, explicit populations.** `summon-rules.countLiveSummons` owns ownership/liveness across map members, fight members, AI actors, and borrowed records. The map layer supplies its temp roster; the fight desk supplies live enemies plus members. Neither module keeps a second `liveSummonsOf`, and the unit case covers all record shapes.
- [x] **Q209** `src/stealth.js:13` [dead-code] `stealth.facingFromYaw` has no production importer. The alleged main.js duplicate is not present; remove the unused export unless a real caller needs it.<br>      ↳ **DONE — removed the unused export and its self-only test.** Production watcher facings come from actor movement or `deriveFacing`; no yaw conversion caller exists to preserve.
- [x] **Q210** `src/ui/screens.js:64` [duplication] The four attributes' labels and blurbs are written out three times across `ui/`, and only one copy derives its key list from `ATTR_KEYS`<br>      ↳ **DONE — attribute presentation is content.** `data/attributes.js` owns the ordered key, label, and blurb records; creation, level-up, and the character sheet render that registry. `stats.js` re-exports its derived key list so numeric callers keep one canonical order without owning UI copy.
...
- [x] **Q217** `src/stats.js:517` [inconsistency] `take-cover` and `pull` land in the class-powers bucket, unlike the third universal verb `shove`<br>      ↳ **DONE — [ratified]** (designer, 2026-08-05: "q217 a"). Universality is the action's own flag now: `universal: true` on shove/take-cover/pull in `data/actions.js`, with `UNIVERSAL_ACTIONS` derived from it and the sorter reading the same flag. There were THREE definitions of "universal" — a hand-written list in hotbar-model, a `type === 'shove'` check in `orderedActionIds`, and the actions file's section header — and only shove satisfied all of them, so both bars filed the other two universal verbs with the class powers. One flag; the list and the sorter cannot disagree with it or each other.
...
- [x] **Q220** `src/looting.js:330` [soc] The Alt-overlay loot icon table is a hardcoded content list in systems code, and it has already drifted — break-room containers get the generic box<br>      ↳ **DONE — the prop definition owns its icon.** Every lootable tile now declares `lootIcon` beside its label and loot table; `looting.js` only projects that data. This matters for the two break-room props in particular: a fridge and a coffee machine share supplies but keep different faces, which a loot-table-keyed icon map could not express. The registry lint requires an icon on every lootable tile.
- [x] **Q221** `src/scene.js:172` [soc] `updateWallFade` still falls back to hardcoded `tileMats.wall` when an opaque solid lacks its own material. Door solids now preserve `solidMat`, narrowing the remaining risk to other custom wall materials.<br>      ↳ DONE — every fadeable wall records its solid and ghost materials; the pure selector test proves a custom solid returns to itself.
...
- [x] **Q225** `tests/unit/hit.test.js:48` [test-gap] hit.test.js - the seeded roll->damage->status test - contains an assertion that cannot fail, a self-comparison, and a hand-copy of combat.js's private damage formula<br>      ↳ DONE — the tests drive production hit resolution and player strike with one seed; hit, damage, rider, miss draw count, costs, effects, and logs are observable outcomes.
- [x] **Q226** `tests/unit/levels.test.js:192` [test-gap] The orphaned-action lint counts any `ammoCost` action as reachable, ignoring `universal: false`<br>      ↳ DONE — ammo only implies universal reachability when `universal` is not explicitly false.
- [x] **Q227** `tests/unit/levels.test.js:207` [test-gap] The registry lint gained no checks for the two AI fields this branch added to enemies.js - `focus` and the `support` descriptor - although the same file lints the equivalent shape for tile topple damage<br>      ↳ DONE — focus is bounded to 0..1 and support validates its heal range, AP, range, uses, and cooldown.
