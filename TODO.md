...
- [ ] **Q041** `src/combat-plans.js:54` [soc] **(carried)** Pure plan modules take a parameter literally named `world` that is combat's host facade, so the contract is duck-typed and unit tests structurally cannot check it<br>      ↳ **THE PROVIDER SIDE IS REACHABLE NOW (2026-08-03).** `world-contract.test.js` pins what the pure rules NEED off the bag and says in its own header that it deliberately cannot check the other side - "main.js is the entry point and importable.test.js excludes it on purpose, so the provider side cannot be reached from node". The facade was a 199-line literal inside `beginCombat`; it is `combat-world.js` now, a plain function returning a plain object, so a test can build one and ask whether it still carries the keys the rules read. **The test itself is not written yet** - that is what is left of this finding.
- [ ] **Q042** `src/combat.js:76` [soc] **(carried)** `startCombat` is one 5,140-line closure - up ~440 lines on this branch - with `armed` mutated at 31 sites across eight subsystems<br>      ↳ **CUT AGAIN 2026-08-04, and this one is the method working as advertised.** `combat-reactions.js`: opportunity attacks (TACTICS_PLAN M2) and the overwatch stance that shares their budget (POWERS_PLAN M5). combat.js 4,762 → 4,596. It qualified on Q037's own test - it OWNS its state, `reactions` and `moveStart`, two of the closure's shared maps that nothing outside writes. The single external write was `reactions.clear()` in the engine's `roundStart`, which is now `clearReactions()`: the engine owns WHEN everyone gets their reaction back, this file owns what that means.
      **The read side was measured first, which is the half Q037 says gets skipped.** `combat-strikes.js` was reverted because a cluster that wrote no shared state still READ the live turn from four directions. This one reads `phase` once, as a getter, and never touches `active` or `armed` - which is why it went in one pass with no e2e round spent hunting a missing name.
      **What did NOT come along is the part worth recording.** `bodyOf`, `standing` and `faceTarget` are declared inside this block and read as though they belong to it. They are used 14, 42 and 20 times across the REST of the closure - generic one-liners that happen to live next to the rule, not part of it. Taking them would have moved a third of the file's vocabulary to pay for a 166-line cut. They arrive as dependencies instead.
      The one behavioural nit: `opportunityStrike` now sits above `notifyStep` rather than below it. Both are hoisted `function` declarations, so the order is readability only - the moved text is otherwise identical modulo the `d.` prefix, verified by normalised diff.
      `tactics` 6/6, `powers` 5/5, `movement` 4/4 - the provoke, the circle, the shove exemption, Frequent Flier and the overwatch that must fire exactly once.
      ↳ **AND `summon-lifecycle.js` (2026-08-04).** A temp's whole stay - `postableNow`, `resolveSummon`, `liveSummonsOf`, `dismissSummon`, `expireSummon`. combat.js 4,596 → 4,514. ~20 deps, one of which is `active` (a single `if (active === target)` in the dismissal), so it is the first cut to touch live turn state at all - as a getter, and that is the whole of it. The descriptor parameter was named `d` in the closure and is `spec` in the module: in a module `d` is the deps bag, the shadow that has shipped twice.
      **It found a live boot-time throw that this branch had already created, and the checker had passed.** `expire: expireSummon` sits in the turn engine's bag at combat.js:953. That was safe for as long as `expireSummon` was a hoisted `function` declaration - and this cut turns it into a `const` **two thousand seven hundred lines below** the bag. An eager read of it there is `Cannot access 'expireSummon' before initialization` at the top of every fight. Wrapped now.
      **Why the check missed it, which is the reusable part.** `deadZoneDeps` matched eager entries at `^\s{4}` - exactly one level of indent - and this entry is nested inside `host: {`, six spaces in. A nested entry is evaluated exactly as eagerly as a top-level one. Widened to `\s{4,}`, and the fix was confirmed the honest way round: the widened check REPORTS the unwrapped line, and goes quiet when it is wrapped.
      **The host was lying too, and that is the more interesting half.** The bag carried a comment reading *"They are declarations, so naming them here before they are written is fine."* True when written, false the moment one of the four stopped being a declaration - and it is precisely the reassurance a reader (or a rewriter) would trust instead of checking. The comment now names which three hoist and why the fourth cannot.
      `summons` 5/5, `charm` 1/1, `ai` 3/3.
...
- [ ] **Q155** `src/combat.js:4829` [soc] **(carried)** The `window.__combat` / `__game` / `__god` debug surfaces are ~420 lines living inside the two god closures, and `__game.classes` hands out the live registry<br>      ↳ **THE main.js HALF IS OUT (2026-08-04).** `__game` (197) and `__god` (147) are `debug-handles.js`, one `createDebugHandles(d)` returning `{ game, god }`; `startGame` 2,868 → 2,592 lines. It is the one block in the closure with **no caller inside the game** - nothing in main.js reads either object - which is why a 66-entry bag is honest here rather than a warning sign: every entry is a read of state somebody else owns, and the only two writes (`party.cash`, the armed god pick) already went through their owner's setter. Everything reassignable arrives as a getter, since a value captured at boot is the pre-picker `null` forever. `god` 4/4, `smoke` 6/6.
      **The shadow trap was live in this block, twice.** Both doors getters destructured `([key, d])`, so a mechanical rewrite makes every `d.` on that line read the deps bag - `open: d.open` normalises to `open: open`. Renamed to `door` rather than shipped.
      **And it unmasked a hole in the checker, which is the part worth keeping.** Removing `walls.filter((w) => ...)` from main.js made the scan report an unbound `w` - from `[\w-]+` in a regex literal at main.js:918. The scan strips comments, strings and templates but never regex literals; a declaration elsewhere in the same file had been masking that since the check was written. Regex literals are stripped now (operand-position only, so plain division survives), and the fix was verified by mutation: an unbound read and an unsupplied `d.` dep are both still caught.
      ↳ **AND THE combat.js HALF (2026-08-04).** `window.__combat` (174 lines) is `combat-handle.js`; `startCombat` 4,762 lines. Same seam, **not** the same shape, and the difference is the part worth reading: `__game` is a window, this one has HANDS. Five members write — `set ap` and `set defended` edit the acting sheet in place and must `refresh()` after (the action bar prices itself off exactly those two), `set forceHit`/`set forceProc` write closure BINDINGS so they go through named setters, and `applyStatus`/`crouch`/`endTurn`/`summonAlly` route through the real verbs rather than imitating them. The `applyStatus` property KEY sitting directly above a call to the `applyStatus` RULE is trap #4 in the checker's own header, and it is why that trap is written down.
      **What this cut actually bought is the checker, and it is worth more than the lines.** Adding `combat.js` to its HOSTS list (it builds bags too, and was never scanned) reported three unbound names — `colleague`, `patches`, `up` — which were prose from ONE log line: nested templates broke the strip, and the leaked backtick then unbalanced the quote passes that ran after it, so the visible hits were not where the bug was. The chain of independent regex replaces could not do the job; it is one left-to-right pass now (`stripNotCode`) with a mode stack, so `${...}` nests arbitrarily and its body is still emitted as code.
      **Then the second half, which was the silent one.** A `get x()` used to add `x` to `declared`, masking that name for the WHOLE FILE — so `get armed() { return armed; }`, precisely the unbound read this tool exists to catch, was invisible. Both handle modules are built almost entirely out of getters named after the dependency they return, so the check was near-blind on exactly the two files it had just been pointed at. Keys are erased from the read stream now instead of declared, and the fix is mutation-verified in both directions.
      **Still open:** `__game.classes` hands out the live `CLASSES` object — a spec that mutates it poisons the rest of the run. Untouched here.
