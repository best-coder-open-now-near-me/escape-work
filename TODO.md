...
- [ ] **Q155** `src/combat.js:4829` [soc] **(carried)** The `window.__combat` / `__game` / `__god` debug surfaces are ~420 lines living inside the two god closures, and `__game.classes` hands out the live registry<br>      ↳ **THE main.js HALF IS OUT (2026-08-04).** `__game` (197) and `__god` (147) are `debug-handles.js`, one `createDebugHandles(d)` returning `{ game, god }`; `startGame` 2,868 → 2,592 lines. It is the one block in the closure with **no caller inside the game** - nothing in main.js reads either object - which is why a 66-entry bag is honest here rather than a warning sign: every entry is a read of state somebody else owns, and the only two writes (`party.cash`, the armed god pick) already went through their owner's setter. Everything reassignable arrives as a getter, since a value captured at boot is the pre-picker `null` forever. `god` 4/4, `smoke` 6/6.
      ...
      **Still open:** combat.js's `window.__combat` (174 lines, combat.js:4580) is the other half, and `__game.classes` still hands out the live `CLASSES` object - a spec that mutates it poisons the rest of the run. Both untouched here.
